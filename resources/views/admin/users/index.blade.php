                        ...
                        <div class="flex items-center justify-between pt-3 border-t border-soft-gray/50">
                            <div class="flex items-center space-x-3">
                                ...
                                <form action="{{ route('admin.users.destroy', $user->id) }}" method="POST" class="sip-delete-user-form" data-nama="{{ $user->nama_lengkap }}">
                                    @csrf @method('DELETE')
                                    <button type="button" onclick="sipModal.deleteForm(this.closest('form'), {title:'Hapus Pengguna', message:'Apakah Anda yakin ingin menghapus pengguna <strong>{{ $user->nama_lengkap }}</strong>? Akun ini tidak dapat dipulihkan.'})" class="p-2 rounded-lg bg-red-50 text-red-600 hover:bg-red-100 hover:text-red-700 transition-colors duration-200" title="Hapus Pengguna">
                            ...
                            <tr class="hover:bg-bg-app/50 transition-colors duration-150">
                                ...
                                <td class="px-4 sm:px-6 py-3 sm:py-4">
                                    <div class="flex items-center space-x-2 sm:space-x-3">
                                        ...
                                        <form action="{{ route('admin.users.destroy', $user->id) }}" method="POST" class="sip-delete-user-form" data-nama="{{ $user->nama_lengkap }}">
                                            @csrf @method('DELETE')
                                            <button type="button" onclick="sipModal.deleteForm(this.closest('form'), {title:'Hapus Pengguna', message:'Apakah Anda yakin ingin menghapus pengguna <strong>{{ $user->nama_lengkap }}</strong>? Akun ini tidak dapat dipulihkan.'})" class="p-1.5 sm:p-2 rounded-lg bg-red-50 text-red-600 hover:bg-red-100 hover:text-red-700 transition-colors duration-200" title="Hapus Pengguna">
